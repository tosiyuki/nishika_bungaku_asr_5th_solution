...
from llm_asr.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX
from llm_asr.model.whisper_encoder import CLIPVisionTower
...
class LlmAsrMetaModel:
    ...
    def initialize_audio_modules(self, model_args):
        ...
        if model_args.audio_encoder_type == "Whisper":
            self.audio_tower = CLIPVisionTower(
                audio_tower, 
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        audio_features = self.get_model().get_audio_tower()(images)       
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if audio_tower is None or images is None or input_ids.shape[1] == 1:
            if past_key_values is not None and audio_tower is not None and images is not None and input_ids.shape[1] == 1:
        ...
        if type(images) is list or images.ndim == 5:
            ...
            audio_features = self.encode_images(concat_images)
            split_sizes = [image.shape[0] for image in images]
            audio_features = torch.split(audio_features, split_sizes, dim=0)
            audio_features = [x.flatten(0, 1).to(self.device) for x in audio_features]
        else:
            audio_features = self.encode_images(images).to(self.device)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                cur_audio_features = audio_features[cur_image_idx]
